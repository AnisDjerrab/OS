...
qemu: bootdisk
	qemu-system-x86_64 -vga cirrus -display gtk -machine pc -m 5G -serial stdio -fda disk.img -no-reboot -d int,guest_errors,cpu_reset,mmu -device ich9-ahci,id=ahci

debug: bootdisk
	qemu-system-x86_64 -vga cirrus -display gtk -machine pc -m 5G -fda disk.img -device ich9-ahci,id=ahci -gdb tcp::26000 -S &
