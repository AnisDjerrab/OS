...
typedef struct{
    ...
    uint32_t ABAR;                  // ptr to the AHCI memory -- we absolutely need it, however, it is specifical to the AHCI devices
} pci_device;
...
void populate_pci_devices_metadata(uint32_t* device_IDs_array, int number_of_found_pci_devices, pci_device* pci_devices);
